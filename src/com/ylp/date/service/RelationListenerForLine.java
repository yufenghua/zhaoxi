...
public class RelationListenerForLine implements ObjListener {
	...
	public void fileAdd(IBaseObj obj) {
		if (obj instanceof IRelation) {
			IRelation relation = (IRelation) obj;
		}
	}
	...
	public void fireUpdate(String id, IBaseObj old, IBaseObj newObj) {
		...
		if (newObj instanceof IRelation) {
			...
			if (relation.getType() == IRelation.TYPE_LINE
					&& relation.getRecognition() == IRelation.RECOG_LINE) {
				Server.getInstance().getLineService()
						.remove(relation.getOne(), relation.getOtherOne());
			}
		}
	}
